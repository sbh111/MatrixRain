...
enum COLOUR	//basically a python dictionary
{
	FG_BLACK = 0x0000,
	FG_DARK_BLUE = 0x0001,
...
private:
	...
	void Create(int w, int h)	//just creates a w x h block of black spaces?
	{
		//seems like this could also be accomplished with a 2d array, used 1d array for less mem?
		nWidth = w;
		nHeight = h;
...
public:
	/*
	how i think these methods are being done:
	...
	so to get that last 0 in the middle, which is in row 4, col 3 -> [3][2]
	you do 3*width + 2 == 17
	*/
	void SetGlyph(int x, int y, short c)
	{
		if (x <0 || x >= nWidth || y < 0 || y >= nHeight)	//if parameters are invalid, then just return
			return;
		else
	...
	//TODO: figure out what SampleGlyph and SampleColor do
	...
	{
		FILE *f = nullptr;
		_wfopen_s(&f, sFile.c_str(), L"wb");	//uses wide char open from iostream, open as "write binary"
		if (f == nullptr)	//if no file exists, returns false
			return false;
